    ...
    return (
      <>
        ...
        <div className="dashboard">
          <div className="container section-sm" style={{ textAlign: "center", padding: "var(--space-16)" }}>
  ...
  if (error || !freelancer) {
    return (
      <>
        ...
        <div className="dashboard">
          <div className="container section-sm" style={{ textAlign: "center", padding: "var(--space-16)" }}>
            <h2 style={{ marginBottom: "var(--space-4)" }}>Freelancer Not Found</h2>
            <p className="text-light" style={{ marginBottom: "var(--space-6)" }}>
              {error || "This profile may not exist or has been deactivated."}
            </p>
            <button className="btn btn-primary" onClick={() => router.push("/talent")}>
              Browse Freelancers
            </button>
          </div>
        </div>
      ...
      <div className="dashboard">
        <div className="container section-sm">
          ...
          <div className="card" style={{ marginBottom: "var(--space-8)" }}>
            <div style={{ display: "flex", gap: "var(--space-8)", alignItems: "start" }}>
              <div
                style={{
                  width: "120px",
                  height: "120px",
                  borderRadius: "50%",
                  backgroundColor: freelancer.avatar ? "transparent" : "var(--color-primary-lightest)",
              ...
              <div style={{ flex: 1 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "start" }}>
                  <div>
                    <h1 style={{ marginBottom: "var(--space-2)" }}>{freelancer.name}</h1>
                    <p className="text-light" style={{ marginBottom: "var(--space-4)" }}>
                      {freelancer.email}
                    </p>
                  </div>
                  <span className="badge badge-success">{freelancer.availabilityStatus || "available"}</span>
                </div>
                ...
                <div style={{ display: "flex", gap: "var(--space-8)", flexWrap: "wrap" }}>
                  ...
                  <div>
                    ...
                    <div style={{ textTransform: "capitalize" }}>{freelancer.experienceLevel || "entry"}</div>
                  </div>
                  <div>
                    <div style={{ fontSize: "var(--text-sm)", color: "var(--color-text-light)", marginBottom: "var(--space-1)" }}>
                      Job Type
                    </div>
                    <div style={{ textTransform: "capitalize" }}>{freelancer.jobTypePreference || "digital"}</div>
                  </div>
                  <div>
                    <div style={{ fontSize: "var(--text-sm)", color: "var(--color-text-light)", marginBottom: "var(--space-1)" }}>
                      Location
                    ...
                    {item.skills?.length > 0 && (
                      <div style={{ marginBottom: "var(--space-3)", display: "flex", gap: "var(--space-2)", flexWrap: "wrap" }}>
                        {item.skills.map((skill) => (
                          <span className="badge badge-success" key={`${item.title}-${skill}`}>
                            {skill}
                          </span>
                        ))}
                      </div>
